...
function getGhostParent() {
  ...
  if (grabbedElement) {
    ...
    if (
      parentElement &&
      !Utils.hasFixedPositionContainingBlockParent(parentElement)
    ) {
...
function getGhostElement(
  ...
  const visualViewportTransform = hasVisualViewportTransform(
  ...
  ghost.style.width =
    (visualViewportTransform ? viewportTransform.size.width : right - left) +
    'px';
  ghost.style.height =
    (visualViewportTransform ? viewportTransform.size.height : bottom - top) +
    'px';
  ...
  return {
    ...
    viewportOffset: visualViewportTransform
      ? viewportTransform.boundingOffset
      : null,
    topLeft: {
      x: left,
      y: top,
